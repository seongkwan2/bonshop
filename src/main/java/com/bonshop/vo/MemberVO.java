...
public class MemberVO {//회원관리 엔티티빈 클래스
	...
	//모든 변경에 대한 전이로서 모든 엔티티빈 상태 변화에 대해서 같이 처리하는 옵션, fetch=FetchType.EAGER는 bs_member와 bs_member_roles 두 테이블을 조회해야 하기
	...
	//bs_member와 bs_member_roles을 둘다 동시에 조회하기 위해서 fetch 설정을 즉시 로딩으로 EAGER 설정을 주어야 에러가 발생하지 않습니다.
